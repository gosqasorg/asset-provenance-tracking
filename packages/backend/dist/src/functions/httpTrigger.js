...
function findDeviceIdFromName(blobName) {
...
export async function calculateDeviceID(key) {
...
function calculateLegacyDeviceID(key) {
    // if key is a string, convert it to a buffer
    key = typeof key === 'string' ? decodeKey(key) : key;
    return fnv1(key);
}
export async function encrypt(key, data, salt) {
...
export async function decrypt(key, salt, encryptedData) {
...
export async function upload(client, deviceKey, data, type, contentType, timestamp, fileName) {
...
async function uploadProvenance(containerClient, deviceKey, timestamp, record, attachments) {
    const attachmentIDs = new Array();
...
async function decryptBlob(client, deviceKey) {
    const props = await client.getProperties();
    const salt = props.metadata?.["gdtsalt"];
    if (!salt)
        throw new Error(`Missing Salt ${client.name}`);
    const timestamp = parseInt(props.metadata?.["gdttimestamp"]);
    if (isNaN(timestamp) || !isFinite(timestamp))
        throw new Error(`Invalid Timestamp ${client.name}`);
...
async function pathExists(containerClient, path) {
...
async function convertLegacyProvenance(containerClient, key) {
    key = typeof key === 'string' ? decodeKey(key) : key;
...
export async function getDecryptedBlob(request, context) {
...
export async function getProvenance(request, context) {
    const deviceKey = decodeKey(request.params.deviceKey);
    const deviceID = await calculateDeviceID(deviceKey);
    ...
    for await (const blob of containerClient.listBlobsFlat({ prefix: `prov/${deviceID}` })) {
        ...
        const provRecord = JSON.parse(json);
        records.push({ ...provRecord, deviceID, timestamp });
    }
    records.sort((a, b) => b.timestamp - a.timestamp);
    return { jsonBody: records };
}
export async function postProvenance(request, context) {
    const deviceKey = decodeKey(request.params.deviceKey);
    const deviceID = await calculateDeviceID(deviceKey);
    ...
    if (typeof provenanceRecord !== 'string') {
        return { status: 404 };
    }
    const record = JSON5.parse(provenanceRecord);
    if (!validateJSON(record)) {
        return { status: 404 };
    }
...
;
export async function getAttachmentName(request, context) {
    const decryptedBlob = await getDecryptedBlob(request, context);
    if (!decryptedBlob) {
        return { status: 404 };
    }
...
;
export async function getStatistics(request, context) {
    const containerExists = await containerClient.exists();
    if (!containerExists) {
        return { jsonBody: [] };
    }
...
;
export async function postEmail(request, context) {
    try {
        ...
        const formData = await request.formData();
        let email;
        if (typeof (email = formData.get('email')) !== 'string') {
...
export async function getVersion(request, context) {
...
export async function getNewDeviceKey(request, context) {
    try {
