...
async function getZipUncompressedSize(blob: Blob): Promise<{ uncompressedSize: number }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    zipStream.on('entry', async (entry) => {
      try {
        // Read the entry data in chunks
        for await (const chunk of entry) {
          totalUncompressedSize += chunk.length;

          if (totalUncompressedSize > MAX_ATTACHMENT_SIZE) {
            exceeded = true;
            zipStream.destroy(new Error(`Uncompressed size exceeds limit of ${MAX_ATTACHMENT_SIZE / (1024 * 1024)}MB`));
            break;
          }
        }
        entry.autodrain(); // Reads and discards the rest of the entry
      } catch (err) {
        reject(err);
      }
    });
    ...
    zipStream.on('error', (err) => {
      if (err.message.includes('limit')) {
        resolve({ uncompressedSize: totalUncompressedSize }); // Return size up to limit
      } else {
        reject(new Error(`Error reading ZIP: ${err.message}`));
      }
    });

    zipStream.on('close', () => {
      if (!exceeded) {
        resolve({ uncompressedSize: totalUncompressedSize }); // Return total size if within limit
      }
    });
...
async function isTarGzip(buffer: Buffer): Promise<boolean> {
  // This function checks if the gzip is a tar.gz
  ...
  return new Promise((resolve, reject) => {
    let chunkCount = 0; // reads in chunks
    let tarFound = false;

    gunzip.on('data', (chunk: Buffer) => {
      chunkCount++;
      // tar headers have "ustar" magic bytes at offset 257
      if (chunk.includes(Buffer.from('ustar'))) {
        tarFound = true;
        gunzip.destroy();
      }
      if (chunkCount > 2) gunzip.destroy(); // don’t read full file
    });

    gunzip.on('close', () => resolve(tarFound));
...
async function getTarSize(buffer: Buffer): Promise<number> {
  // Gets uncompressed size of a .tar.gz by streaming and parsing TAR entries under or equal to MAX_ATTACHMENT_SIZE
  return new Promise((resolve, reject) => {
    const gunzip = createGunzip();
    ...
    extract.on('entry', (header, fileStream, next) => {
      ...
      if (total > MAX_ATTACHMENT_SIZE) {
        // Exceeds limit, abort
        gunzip.destroy(new Error('Uncompressed size exceeds limit'));
        return;
      }
      fileStream.resume(); // discards contents and resumes stream
      next();
    });
...
async function getGzipUncompressedSize(blob: Blob): Promise<{ uncompressedSize: number }> {
  // Handles gzip files, including .tar.gz
  const buffer = Buffer.from(await blob.arrayBuffer());
    try {
      // GZIP stores uncompressed size in the last 4 bytes 
      if (buffer.length < 4) {
        throw new Error('Attachment is too small to be a valid GZIP file');
      }

      if (buffer[0] !== 0x1f || buffer[1] !== 0x8b) {
        // checks for GZIP header
        throw new Error('Not a valid GZIP file');
      }

      // Read the last 4 bytes
      const sizeBuffer = buffer.subarray(-4);
      const uncompressedSize = sizeBuffer.readUInt32LE(0); // Read as unsigned 32-bit int

      const isize = uncompressedSize;

      // Check for tar header magic "ustar" in the decompressed start of stream
      const maybeTar = await isTarGzip(buffer);
      
      if (!maybeTar && isize > 0) {
        return { uncompressedSize: isize };
      }
      //If it is a .tar.gz, stream-decompress and get size of tar contents
      const total = await getTarSize(buffer);
      return { uncompressedSize: total };
    } catch (error) {
      throw new Error(`GZIP size check failed: ${error.message}`);
    }
}
...
async function uploadProvenance(containerClient: ContainerClient, deviceKey: Uint8Array, timestamp: number, record: any, attachments: NamedBlob[]): Promise<{ record: string; attachments: NamedBlob[]; }> {
  ...
  for (const attach of attachments) {
    ...
    const attachmentExtension = attach.name?.split('.').pop()?.toLowerCase();
    let attachSize = attach.blob.size;

    console.log(`Processing attachment: ${attach.name}, extension: ${attachmentExtension}, original size: ${attachSize}`);

    // Checks for ZIP or GZIP attachments and gets the uncompressed size
    if (attachmentExtension === 'zip') {
      try {
        const zipInfo = await getZipUncompressedSize(attach.blob);
        attachSize = zipInfo.uncompressedSize;
        console.log(`ZIP uncompressed size: ${attachSize}`);
      } catch (error) {
        // If it's not a valid ZIP, use the original blob size
        console.log(`ZIP size check failed: ${error.message}, using original size: ${attach.blob.size}`);
        attachSize = attach.blob.size;
      }
    } else if (attachmentExtension === 'gz'|| attachmentExtension.endsWith('.gzip') || attachmentExtension.endsWith('.tgz')) {
      try {
        const gzipInfo = await getGzipUncompressedSize(attach.blob);
        attachSize = gzipInfo.uncompressedSize;
        console.log(`GZIP uncompressed size: ${attachSize}`);
      } catch (error) {
        // If it's not a valid GZIP, use the original blob size
        console.log(`GZIP size check failed: ${error.message}, using original size: ${attach.blob.size}`);
        attachSize = attach.blob.size;
      }
    }
