...
export async function encrypt(
  ...
  data: BufferSource,
  salt?: Uint8Array
): Promise<{ salt: Uint8Array; encryptedData: Uint8Array }> {
...
): Promise<{ record: string; attachments: NamedBlob[] }> {
...
async function decryptBlob(client: BlockBlobClient, deviceKey: Uint8Array): Promise<DecryptedBlob> {
  ...
  const encodedName =
    encryptedName.length > 0
      ? await decrypt(deviceKey, saltBuffer, fromHex(encryptedName))
      : undefined;
...
async function convertLegacyProvenance(containerClient: ContainerClient, key: string | Uint8Array) {
  ...
  for await (const blob of containerClient.listBlobsFlat({
    prefix: `legacy/prov/${legacyDeviceID}`
  })) {
    ...
    for (const attachment of attachmentIDs) {
      const attachmentClient = containerClient.getBlockBlobClient(
        `legacy/attach/${attachment.attachmentID}`
      );
...
async function upgradeProvenance(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  const deviceKey = decodeKey(request.params.deviceKey);
