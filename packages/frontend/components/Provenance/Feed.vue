  ...
  <div>
    <div v-for="(report, index) in provenance" class="report-box" :style="{ border }">
      ...
      <div class="mb-1 tag-container">
        <span
          class="tag"
          v-for="tag in report.record.tags"
          v-bind:style="
            'color: ' + textColorForTag(tag) + '; background-color: ' + getColorForTag(tag) + ';'
          "
        >
          {{ tag }}</span
        >
      ...
      >
        <img
          ...
          @click="modalImage = attachment.url"
        />
    ...
    async fetchAttachmentsForReport(report, index) {
      try {
        if (report.attachments.length > 0) {
          ...
          const attachmentPromises = report.attachments.map((attachmentID) =>
            getAttachment(baseUrl, this.recordKey, attachmentID)
          );
