...
<template>
  <form enctype="multipart/form-data" class="p-3" id="record-form" @submit.prevent="submitForm">
    ...
    <div>
      ...
      <h4 class="mt-3 mb-2" for="children-keys">
        Number of Grouped Records (optional)
        <input
          type="number"
          class="form-inline"
          id="children-keys"
          v-model="childrenKeys"
          min="0"
          max="500"
      ...
      <h4 class="p-1 my-0">
        <input
          ...
          @change="displayFields"
        />
        Customize Grouped Record Titles?
      </h4>
      ...
      <h4 class="p-1">
        <input v-model="isChecked" type="checkbox" class="form-check-input" /> I'm open to providing
        feedback on my experience with GDT
      </h4>
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        const response = await postProvenance(
          deviceKey,
          {
            blobType: 'deviceInitializer',
