...
const consent = require('./shared/telemetry.lfCiUK2w.cjs');
...
const parseGitConfig__default = /*#__PURE__*/_interopDefaultCompat(parseGitConfig);
...
async function getGitRemote(rootDir) {
  try {
    const parsed = await parseGitConfig__default({ cwd: rootDir });
    if (parsed) {
