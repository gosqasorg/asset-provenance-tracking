...
exports.formatDetectedFolders = exports.printSupportedFrameworks = exports.isDescendantPath = exports.detectFrameworks = exports.detectProjectFolders = exports.detectDbConfigFiles = exports.generateConfiguration = void 0;
...
async function generateConfiguration(app, api, dataApi) {
    let config = {
...
async function detectDbConfigFiles(projectPath = ".") {
    // Detect all the "staticwebapp.database.config.json" with valid "database-type"
    const projectFiles = await getFiles(projectPath);
    const dbConfigFilePaths = projectFiles.filter((f) => f.endsWith(constants_1.DATA_API_BUILDER_DEFAULT_CONFIG_FILE_NAME));
    let dbConfigFiles = await Promise.all(dbConfigFilePaths.map(async (f) => {
        var _a;
        const contains = await (0, utils_1.safeReadJson)(f);
        let result = {
            rootPath: path_1.default.dirname(f),
            databaseType: (_a = contains === null || contains === void 0 ? void 0 : contains["data-source"]) === null || _a === void 0 ? void 0 : _a["database-type"],
        };
        return result;
    }));
...
exports.detectDbConfigFiles = detectDbConfigFiles;
