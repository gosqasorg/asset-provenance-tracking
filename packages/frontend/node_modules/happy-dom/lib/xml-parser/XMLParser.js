                ...
                switch (readState) {
                    case MarkupReadStateEnum.startOrEndTag:
                        ...
                        if (match[1]) {
                            ...
                            const qualifiedName = namespaceURI === NamespaceURI.svg && SVGElementConfig[name]
                    ...
                    case MarkupReadStateEnum.insideStartTag:
                        ...
                        if (match[7] || match[8]) {
                            ...
                            if (!!attributeString) {
                                ...
                                while ((attributeMatch = attributeRegexp.exec(attributeString))) {
                                    if ((attributeMatch[1] && attributeMatch[2]) ||
                                        ...
                                        if (newNode[PropertySymbol.namespaceURI] === NamespaceURI.svg) {
                                            // In XML and SVG namespaces, the attribute "xmlns" should be set to the "http://www.w3.org/2000/xmlns/" namespace.
                                            const attributeItem = document.createAttributeNS(name === 'xmlns' ? NamespaceURI.xmlns : null, name);
                            ...
                            if (hasAttributeStringEnded) {
                                const config = HTMLElementConfig[newNode[PropertySymbol.localName]];
                                const localName = StringUtility.asciiLowerCase(newNode[PropertySymbol.localName]);
